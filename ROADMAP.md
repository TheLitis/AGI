...
- B0 (сделано): стабильный tool-loop для `proc_*_loop`, action-mask как реалистичный UI, отдельная метрика `unmasked` для проверки интернализации.
...
- `trainer.train_policy_a2c()`: штраф за `mean_invalid_mass` (вероятность на invalid actions) — заставляет политику “выучить” запреты, а не полагаться на маску (настраивается флагом `--invalid-action-coef`).
