...
- **Туллуп (первые шаги)**: `RepoToolEnv` запускает реальный `pytest` в песочнице; действия дискретные, но есть микро-редактирование (`RepoEdit`) и процедурные сценарии `proc_*` (меньше ручной разметки).
...
Есть: `ComputerEnv` (симуляция), `RepoToolEnv` (реальный pytest, но дискретные патчи).  
Не хватает: открытого пространства действий (редактирование кода, команды, поиск по репо), устойчивых длинных workflow (план → исполнение → проверка → дебаг → повтор), и “agent state” (журнал, гипотезы, прогресс).
...
Есть: тесты (`tests/`), свипы (`run_sweep.py`), анализ (`analyze_*.py`), чекпоинты (`checkpointing.py`) и возобновление (`--resume-from`).  
Не хватает: стабильного протокола бенчмарков “от простого к сложному”, и метрик уровня “способность” (capabilities) поверх голого reward.
...
Цель: “агент реально решает задачи в репозитории”.
- Расширить `RepoToolEnv` в сторону не-дискретных действий (хотя бы: выбор/композиция патчей, редактирование строк, поиск по ошибкам).
- Добавить задачи на multi-file, регрессы, refactor.
