...
def test_run_suite_resume_continues_from_run_cache(tmp_path, monkeypatch):
    calls = []
    ...
    suite = bench.SuiteSpec(
        name="core",
        cases=[
            bench.BenchCase(name="grid_a", env_type="gridworld"),
            bench.BenchCase(name="grid_b", env_type="gridworld"),
        ],
        implemented=True,
    )
    report = {
                ...
                "run_cache": [
                    {
                        ...
                        "seed": 0,
                        "status": "ok",
                        "eval": {"mean_return": 1.0, "test_mean_return": 1.0},
                        "result": {"stage_metrics": {"eval_after_stage4_self": {"mean_return": 1.0}}},
                    }
                ],
            }
        ],
    }
    report_path = Path(tmp_path) / "resume_suite_report.json"
    ...
    assert result["status"] == "ok"
    # only missing case (grid_b) should be executed
    assert len(calls) == 1
