...
def test_repo_tool_env_procedural_regression_task_is_solvable(tmp_path):
    tasks = build_repo_taskset(["proc_regression"])
    ...
    env = RepoToolEnv(task_set=tasks, config=cfg, env_id=0, env_name="repo_proc_regression", seed=0)
    ...
    raise AssertionError("procedural regression task did not become solvable within the candidate search budget")
